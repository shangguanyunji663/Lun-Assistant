...
> 本轮事由：结合 RAG 知识库系统 / Agent 运维助手两个参考项目的设计思路，把论匠升级为**智能论文知识库助手**（每个论文项目自带私有知识库 + 多步规划执行）。P0 与 P1 阶段落地完成。
...
| 结构化产物    | 文献综述初稿 / 开题报告 / 答辩大纲（证据检索 + 模板生成 + 治理工具）            | 第四轮实现于 [artifacts](file:///d:/PythonProject/Lun-Assistant/services/agent/artifacts.py)，**第六轮已下沉至** [services/governance/artifacts.py](file:///d:/PythonProject/Lun-Assistant/services/governance/artifacts.py)（打破 agent↔governance 双向依赖） |
| 量化验收     | 七大必测场景自动化回归评测（16 项断言全 PASS）                         | [regression](file:///d:/PythonProject/Lun-Assistant/evals/regression.py)                                                                                                                                                                                                  |
| 知识库 API  | 上传 / 列表 / 删除 / 库内检索（project / hybrid 双模式）           | 第四轮实现于 [projects 路由](file:///d:/PythonProject/Lun-Assistant/api/projects/router.py)，**第六轮已拆分至** [api/knowledge/router.py](file:///d:/PythonProject/Lun-Assistant/api/knowledge/router.py) |
