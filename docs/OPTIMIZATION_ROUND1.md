# 优化记录 · 第一轮（P0 批次：性能 / 安全 / 体验）

> ⚠️ **变更标注（2026-09-02 · 文档治理轮）**：本文档为混合轮次（主体 backend），其中「助手消息 Markdown 渲染（用户体验）」一节属前端功能，前端改进统一归档见 [`frontend-versions/README.md`](./frontend-versions/README.md)。
...
**修复**：[services/llm/provider.py](../services/llm/provider.py) 增加模块级 `_client_cache`，
...
**问题**：[api/middleware/audit.py](../api/middleware/audit.py) 注释声称 fire-and-forget，
...
未跟踪文件从 2370 个降至 111 个（此前 node\_modules 约 2200 个依赖文件被误纳入）。
...
- **P1**：trace\_spans/audit\_logs 定期清理（表膨胀）、检索延迟优化（意图分层自适应改写开关，
